...
struct Config
{
  public:
    ...
    int iRenderingMode;    // 0 = non-buffered rendering 1 = buffered rendering 2 = Read Framebuffer to memory (CPU) 3 =
                           // Read Framebuffer to memory (GPU)
    int  iTexFiltering;    // 1 = off , 2 = nearest , 3 = linear , 4 = linear(CG)
    int  iBufFilter;       // 1 = linear, 2 = nearest
    bool bPartialStretch;
    bool bStretchToDisplay;
    bool bSmallDisplay;     // Useful on large tablets with touch controls to not overlap the image. Temporary setting -
                            // will be replaced by more comprehensive display size settings.
    ...
    std::string upgradeMessage;    // The actual message from the server is currently not used, need a translation
                                   // mechanism. So this just acts as a flag.
