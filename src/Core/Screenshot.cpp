...
static bool WriteScreenshotToJPEG(const char *filename, int width, int height, int num_channels,
                                  const uint8 *image_data, const jpge::params &comp_params)
{
...
static bool WriteScreenshotToPNG(png_imagep image, const char *filename, int convert_to_8bit, const void *buffer,
...
static const u8 *ConvertBufferTo888RGB(const GPUDebugBuffer &buf, u8 *&temp)
{
    ...
    if (buf.GetFlipped() && buf.GetFormat() == GPU_DBG_FORMAT_888_RGB) {
        ...
        for (u32 y = 0; y < buf.GetHeight(); y++) {
            memcpy(temp + y * buf.GetStride() * 3, buffer + (buf.GetHeight() - y - 1) * buf.GetStride() * 3,
                   buf.GetStride() * 3);
        }
        buffer = temp;
