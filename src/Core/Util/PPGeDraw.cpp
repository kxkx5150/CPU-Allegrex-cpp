...
void __PPGeInit()
{
    ...
    DEBUG_LOG(SCEGE, "PPGe drawing library initialized. DL: %08x Data: %08x Atlas: %08x (%i) Args: %08x", dlPtr,
              dataPtr, atlasPtr, atlasSize, listArgs.ptr);
}
...
static AtlasTextMetrics BreakLines(const char *text, const AtlasFont &atlasfont, float x, float y, int align,
                                   float scale, int wrapType, float wrapWidth, bool dryRun)
{
...
void PPGeMeasureText(float *w, float *h, int *n, const char *text, float scale, int WrapType, int wrapWidth)
{
...
{
    if (x)
        *x = char_lines_metrics.x;
    if (y)
        *y = char_lines_metrics.y;
    if (w)
        *w = char_lines_metrics.maxWidth;
    if (h)
        *h = char_lines_metrics.lineHeight;
    if (n)
        *n = char_lines_metrics.numLines;
}
...
void PPGeDrawImage(float x, float y, float w, float h, float u1, float v1, float u2, float v2, int tw, int th,
                   u32 color)
{
    if (!dlPtr)
        return;
...
PPGeImage::PPGeImage(const std::string &pspFilename) : filename_(pspFilename), texture_(0)
{
}

PPGeImage::PPGeImage(u32 pngPointer, size_t pngSize) : filename_(""), png_(pngPointer), size_(pngSize), texture_(0)
{
}
...
bool PPGeImage::Load()
{
    ...
    if (filename_.empty()) {
        ...
        success =
            pngLoadPtr((const unsigned char *)&pngData[0], pngData.size(), &width_, &height_, &textureData, false);
    }
    if (!success) {
