...
DirectoryFileSystem::DirectoryFileSystem(IHandleAllocator *_hAlloc, std::string _basePath, int _flags)
    : basePath(_basePath), flags(_flags)
{
...
size_t DirectoryFileSystem::ReadFile(u32 handle, u8 *pointer, s64 size, int &usec)
{
    ...
    if (iter != entries.end()) {
        if (size < 0) {
            ERROR_LOG_REPORT(FILESYS, "Invalid read for %lld bytes from disk %s", size,
                             iter->second.guestFilename.c_str());
            return 0;
        }
    ...
    if (p.mode == p.MODE_READ) {
        ...
        for (u32 i = 0; i < num; i++) {
            ...
            if (position != entry.hFile.Seek(position, FILEMOVE_BEGIN)) {
                ERROR_LOG(FILESYS, "Failed to restore seek position while loading state: %s",
                          entry.guestFilename.c_str());
                continue;
            }
            if (s >= 2) {
