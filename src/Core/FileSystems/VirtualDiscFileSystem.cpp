    ...
    if (iter != entries.end()) {
        ...
        switch (entry.type) {
            ...
            case VFILETYPE_LBN: {
                switch (type) {
                    case FILEMOVE_BEGIN:
                        entry.curOffset = position;
                        break;
                    case FILEMOVE_CURRENT:
                        entry.curOffset += position;
                        break;
                    case FILEMOVE_END:
                        entry.curOffset = entry.size + position;
                        break;
                }
            ...
            case VFILETYPE_ISO: {
                switch (type) {
                    ...
                    case FILEMOVE_CURRENT:
                        ...
                        break;
                    case FILEMOVE_END:
                        entry.curOffset = currentBlockIndex + position;
                        break;
                }
...
size_t VirtualDiscFileSystem::ReadFile(u32 handle, u8 *pointer, s64 size, int &usec)
{
    ...
    if (iter != entries.end()) {
        ...
        if (iter->second.type == VFILETYPE_ISO) {
            ...
            if (fileIndex == -1) {
                ERROR_LOG(FILESYS, "VirtualDiscFileSystem: Reading from unknown address in %08x at %08llx", handle,
                          iter->second.curOffset);
                return 0;
            }
            ...
            if (!success) {
                ERROR_LOG(FILESYS, "VirtualDiscFileSystem: Error opening file %s",
