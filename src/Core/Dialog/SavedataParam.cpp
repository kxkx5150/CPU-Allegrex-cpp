...
{
    u32 handle =
        pspFileSystem.OpenFile(filename, (FileAccess)(FILEACCESS_WRITE | FILEACCESS_CREATE | FILEACCESS_TRUNCATE));
    if (handle == 0)
        return false;
...
SavedataParam::SavedataParam()
    : pspParam(0), selectedSave(0), saveDataList(0), noSaveIcon(0), saveDataListCount(0), saveNameListDataCount(0)
{
}
...
bool SavedataParam::Save(SceUtilitySavedataParam *param, const std::string &saveDirName, bool secureMode)
{
    ...
    if (param->dataBuf.IsValid() && g_Config.bEncryptSave && secureMode) {
        ...
        if (EncryptData(decryptMode, cryptedData, &cryptedSize, &aligned_len, cryptedHash,
                        (HasKey(param) ? param->key : 0)) != 0) {
            I18NCategory *err = GetI18NCategory("Error");
...
bool SavedataParam::LoadSaveData(SceUtilitySavedataParam *param, const std::string &saveDirName,
                                    ...
                                    int prevCryptMode, bool &saveDone)
{
    ...
    if (decryptMode != prevCryptMode) {
        if (prevCryptMode == 1 && param->key[0] == 0) {
            ...
            WARN_LOG(SCEUTILITY, "Savedata loading with hashmode %d instead of detected %d", prevCryptMode,
                     decryptMode);
            hasKey      = false;
            ...
            if (decryptMode == 5 && prevCryptMode == 3) {
                WARN_LOG(SCEUTILITY, "Savedata loading with detected hashmode %d instead of file's %d", decryptMode,
                         prevCryptMode);
            } else {
                WARN_LOG_REPORT(SCEUTILITY, "Savedata loading with detected hashmode %d instead of file's %d",
                                decryptMode, prevCryptMode);
            }
        }
    }
...
void SavedataParam::LoadFile(const std::string dirPath, const std::string filename,
...
int SavedataParam::EncryptData(unsigned int mode, unsigned char *data, int *dataLen, int *alignedLen,
...
int SavedataParam::DecryptSave(unsigned int mode, unsigned char *data, int *dataLen, int *alignedLen,
...
int SavedataParam::BuildHash(unsigned char *output, unsigned char *data, unsigned int len, unsigned int alignedLen,
                             int mode, unsigned char *cryptkey)
{
    pspChnnlsvContext1 ctx1;
...
int SavedataParam::GetSizes(SceUtilitySavedataParam *param)
{
    ...
    if (param->msData.IsValid()) {
        const std::string gameName(param->msData->gameName,
                                   strnlen(param->msData->gameName, sizeof(param->msData->gameName)));
        const std::string saveName(param->msData->saveName,
                                   strnlen(param->msData->saveName, sizeof(param->msData->saveName)));
        ...
        if (finfo.exists) {
            ...
            for (auto it = listing.begin(), end = listing.end(); it != end; ++it) {
                param->msData->info.usedClusters +=
                    (it->size + (u32)MemoryStick_SectorSize() - 1) / (u32)MemoryStick_SectorSize();
            }
...
int SavedataParam::GetFilesList(SceUtilitySavedataParam *param)
{
    ...
    if (fileList->secureEntries.IsValid() && fileList->maxSecureEntries > 99) {
        ERROR_LOG_REPORT(SCEUTILITY, "SavedataParam::GetFilesList(): too many secure entries, %d",
    ...
    if (fileList->normalEntries.IsValid() && fileList->maxNormalEntries > 8192) {
        ERROR_LOG_REPORT(SCEUTILITY, "SavedataParam::GetFilesList(): too many normal entries, %d",
    ...
    if (sceKernelGetCompiledSdkVersion() >= 0x02060000) {
        if (fileList->systemEntries.IsValid() && fileList->maxSystemEntries > 5) {
            ERROR_LOG_REPORT(SCEUTILITY, "SavedataParam::GetFilesList(): too many system entries, %d",
