...
struct ConfigSetting
{
    ...
    ConfigSetting(bool v) : ini_(""), type_(TYPE_TERMINATOR), report_(false), save_(false), perGame_(false)
    {
        ptr_.b = NULL;
        cb_.b  = NULL;
    }
...
static ConfigSetting generalSettings[] = {
    ...
    ConfigSetting("ScreenshotsAsPNG", &g_Config.bScreenshotsAsPNG, true, true, true),
    ...
    ConfigSetting("WindowWidth", &g_Config.iWindowWidth,
                  0),    // 0 will be automatically reset later (need to do the AdjustWindowRect dance).
...
static ConfigSectionSettings sections[] = {
    {"General", generalSettings},         {"CPU", cpuSettings},
    {"Graphics", graphicsSettings},       {"Sound", soundSettings},
    {"Control", controlSettings},         {"Network", networkSettings},
    {"SystemParam", systemParamSettings}, {"Debugger", debuggerSettings},
    {"SpeedHacks", speedHackSettings},    {"JIT", jitSettings},
...
bool Config::loadGameConfig(const std::string &pGameId)
{
    ...
    if (!hasGameConfig(pGameId)) {
        INFO_LOG(LOADER, "Failed to read %s. No game-specific settings found, using global defaults.",
