...
u32 JitBreakpoint()
{
    ...
    if (USE_JIT_MISSMAP) {
        ...
        std::transform(notJitOps.begin(), notJitOps.end(), std::inserter(notJitSorted, notJitSorted.begin()),
                       flip_pair<std::string, u32>);
...
const u8 *Jit::DoJit(u32 em_address, JitBlock *b)
{
    ...
    if (js.lastContinuedPC == 0)
        ...
        blocks.ProxyBlock(js.blockStart, js.lastContinuedPC, (js.compilerPC - js.lastContinuedPC) / sizeof(u32),
                          GetCodePtr());
...
void Jit::WriteExit(u32 destination, int exit_num)
{
    ...
    if (!Memory::IsValidAddress(destination)) {
        ERROR_LOG_REPORT(JIT, "Trying to write block exit to illegal destination %08x: pc = %08x", destination,
                         currentMIPS->pc);
    }
