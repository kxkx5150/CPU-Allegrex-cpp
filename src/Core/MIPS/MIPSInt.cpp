...
void Int_RType3(MIPSOpcode op)
{
    ...
    switch (op & 63) {
        ...
        case 39:
            ...
            break;    // nor
        case 42:
            R(rd) = (s32)R(rs) < (s32)R(rt);
            break;    // slt
        case 43:
            R(rd) = R(rs) < R(rt);
            break;    // sltu
        case 44:
            R(rd) = ((s32)R(rs) > (s32)R(rt)) ? R(rs) : R(rt);
            break;    // max
        case 45:
            R(rd) = ((s32)R(rs) < (s32)R(rt)) ? R(rs) : R(rt);
            break;    // min
...
void Int_ITypeMem(MIPSOpcode op)
{
    ...
    switch (op >> 26) {
        case 32:
            R(rt) = (u32)(s32)(s8)Memory::Read_U8(addr);
            break;    // lb
        case 33:
            R(rt) = (u32)(s32)(s16)Memory::Read_U16(addr);
            break;    // lh
        case 35:
            R(rt) = Memory::Read_U32(addr);
            break;    // lw
    ...
    switch (op & 0x3f) {
        ...
        case 36:
            ...
            switch (currentMIPS->fcr31 & 3) {
                case 0:
                    FsI(fd) = (int)round_ieee_754(F(fs));
                    break;    // RINT_0
                case 1:
                    FsI(fd) = (int)F(fs);
                    break;    // CAST_1
