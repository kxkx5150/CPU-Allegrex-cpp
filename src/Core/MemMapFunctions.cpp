...
u8 *GetPointer(const u32 address)
{
    if ((address & 0x3E000000) == 0x08000000) {
        ...
        ERROR_LOG(MEMMAP, "Unknown GetPointer %08x PC %08x LR %08x", address, currentMIPS->pc,
                  currentMIPS->r[MIPS_REG_RA]);
        static bool reported = false;
        if (!reported) {
            Reporting::ReportMessage("Unknown GetPointer %08x PC %08x LR %08x", address, currentMIPS->pc,
                                     currentMIPS->r[MIPS_REG_RA]);
            reported = true;
        }
...
template <typename T> inline void ReadFromHardware(T &var, const u32 address)
{
    ...
    if ((address & 0x3E000000) == 0x08000000) {
        ...
        if (g_Config.bJit && g_Config.bIgnoreBadMemAccess) {
            ...
            WARN_LOG(MEMMAP, "ReadFromHardware: Invalid address %08x PC %08x LR %08x", address, currentMIPS->pc,
                     currentMIPS->r[MIPS_REG_RA]);
        }
        static bool reported = false;
        if (!reported) {
            Reporting::ReportMessage("ReadFromHardware: Invalid address %08x near PC %08x LR %08x", address,
...
template <typename T> inline void WriteToHardware(u32 address, const T data)
{
    ...
    if ((address & 0x3E000000) == 0x08000000) {
        ...
        *(T *)GetPointerUnchecked(address) = data;
        ...
        if (g_Config.bJit && g_Config.bIgnoreBadMemAccess) {
            ...
            WARN_LOG(MEMMAP, "WriteToHardware: Invalid address %08x	PC %08x LR %08x", address, currentMIPS->pc,
                     currentMIPS->r[MIPS_REG_RA]);
        }
        static bool reported = false;
        if (!reported) {
            Reporting::ReportMessage("WriteToHardware: Invalid address %08x near PC %08x LR %08x", address,
                                     currentMIPS->pc, currentMIPS->r[MIPS_REG_RA]);
            reported = true;
        }
        if (!g_Config.bIgnoreBadMemAccess) {
