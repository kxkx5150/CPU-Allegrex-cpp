...
static const char *altBootNames[] = {
    "disc0:/PSP_GAME/SYSDIR/EBOOT.OLD",        "disc0:/PSP_GAME/SYSDIR/EBOOT.DAT",
    "disc0:/PSP_GAME/SYSDIR/EBOOT.BI",         "disc0:/PSP_GAME/SYSDIR/EBOOT.LLD",
    "disc0:/PSP_GAME/SYSDIR/OLD_EBOOT.BIN",    "disc0:/PSP_GAME/SYSDIR/EBOOT.123",
    "disc0:/PSP_GAME/SYSDIR/EBOOT_LRC_CH.BIN", "disc0:/PSP_GAME/SYSDIR/BOOT0.OLD",
    "disc0:/PSP_GAME/SYSDIR/BOOT1.OLD",        "disc0:/PSP_GAME/SYSDIR/BINOT.BIN",
    "disc0:/PSP_GAME/SYSDIR/EBOOT.FRY",        "disc0:/PSP_GAME/SYSDIR/EBOOT.Z.Y",
    "disc0:/PSP_GAME/SYSDIR/EBOOT.LEI",        "disc0:/PSP_GAME/SYSDIR/EBOOT.DNR",
    "disc0:/PSP_GAME/SYSDIR/DBZ2.BIN",         "disc0:/PSP_GAME/SYSDIR/ss.RAW",
};
...
bool Load_PSP_ISO(FileLoader *fileLoader, std::string *error_string)
{
    ...
    if (fileInfo.exists) {
        ...
        if (g_paramSFO.ReadSFO(paramsfo)) {
            char title[1024];
            sprintf(title, "%s : %s", g_paramSFO.GetValueString("DISC_ID").c_str(),
    ...
    if ((fd = pspFileSystem.OpenFile(bootpath, FILEACCESS_READ)) != 0) {
        ...
        if (memcmp(head, "~PSP", 4) == 0 || memcmp(head,
                                                   "\x7F"
                                                   "ELF",
                                                   4) == 0) {
            hasEncrypted = true;
