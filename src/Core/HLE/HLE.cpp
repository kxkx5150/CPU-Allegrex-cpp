...
void WriteFuncStub(u32 stubAddr, u32 symAddr)
{
    ...
    // Note: doing that, we can't trace external module calls, so maybe something else should be done to debug more
    // efficiently Perhaps a syscall here (and verify support in jit), marking the module by uid (debugIdentifier)?
...
const static u32   deadbeefRegs[12] = {0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF, 0xDEADBEEF,
...
const HLEFunction *GetSyscallInfo(MIPSOpcode op)
{
    ...
    if (funcnum == 0xfff) {
        ERROR_LOG(HLE, "Unknown syscall: Module: %s",
                  modulenum > (int)moduleDB.size() ? "(unknown)" : moduleDB[modulenum].name);
        return NULL;
    }
    ...
    if (funcnum >= moduleDB[modulenum].numFunctions) {
        ERROR_LOG(HLE, "Syscall had bad function number %i in module %i - probably executing garbage", funcnum,
                  modulenum);
        return NULL;
    }
...
{
    double start = 0.0;    // need to initialize to fix the race condition where g_Config.bShowDebugStats is enabled in
                           // the middle of this func.
    if (g_Config.bShowDebugStats) {
        time_update();
        start = time_now_d();
    }
