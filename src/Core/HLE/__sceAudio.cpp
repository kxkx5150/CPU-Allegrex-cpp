...
inline void __AudioWakeThreads(AudioChannel &chan, int result, int step)
{
    ...
    for (size_t w = 0; w < chan.waitingThreads.size(); ++w) {
        ...
        if (waitInfo.numSamples <= 0 && waitID != 0) {
            ...
            u32 ret =
                result == 0 ? __KernelGetWaitValue(waitInfo.threadID, error) : SCE_ERROR_AUDIO_CHANNEL_NOT_RESERVED;
...
void __AudioUpdate()
{
    ...
    for (u32 i = 0; i < PSP_AUDIO_CHANNEL_MAX + 1; i++) {
        ...
        if (hwBlockSize * 2 > (int)chans[i].sampleQueue.size()) {
            ERROR_LOG(SCEAUDIO, "Channel %i buffer underrun at %i of %i", i, (int)chans[i].sampleQueue.size() / 2,
                      hwBlockSize);
        }
        ...
        if (firstChannel) {
            ...
            // Surprisingly hard to SIMD efficiently on SSE2 due to lack of 16-to-32-bit sign extension. NEON should be
            // straight-forward though, and SSE4.1 can do it nicely. Actually, the cmple/pack trick should work fine...
            for (size_t s = 0; s < sz1; s++)
                mixBuffer[s] += buf1[s];
            if (buf2) {
                for (size_t s = 0; s < sz2; s++)
