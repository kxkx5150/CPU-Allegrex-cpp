...
class FileNode : public KernelObject {
  public:
    FileNode()
        : callbackID(0), callbackArg(0), asyncResult(0), hasAsyncResult(false), pendingAsyncResult(false),
          sectorBlockMode(false), closePending(false), npdrm(0), pgdInfo(NULL)
    {
    }
    ~FileNode()
    {
    ...
    {
        return PPSSPP_KERNEL_TMID_File;
    }
    int GetIDType() const override
    {
        return PPSSPP_KERNEL_TMID_File;
    }
    ...
    if (fd < PSP_MIN_FD || fd >= PSP_COUNT_FDS) {
        ...
        if (f) {
            ...
            for (size_t i = 0; i < f->waitingThreads.size(); ++i) {
                HLEKernel::ResumeFromWait(f->waitingThreads[i], WAITTYPE_ASYNCIO, f->GetUID(),
                                          (int)SCE_KERNEL_ERROR_WAIT_DELETE);
            }
...
static void __IoSyncNotify(u64 userdata, int cyclesLate)
{
    ...
    f->waitingSyncThreads.erase(std::remove(f->waitingSyncThreads.begin(), f->waitingSyncThreads.end(), threadID),
                                f->waitingSyncThreads.end());
}
...
{
    auto result = HLEKernel::WaitEndCallback<FileNode, WAITTYPE_ASYNCIO, SceUID>(threadID, prevCallbackId, -1,
                                                                                 __IoCheckAsyncWait);
...
static u32 sceIoAssign(u32 alias_addr, u32 physical_addr, u32 filesystem_addr, int mode, u32 arg_addr, int argSize)
{
    ...
    WARN_LOG_REPORT(SCEIO, "sceIoAssign(%s, %s, %s, %s, %08x, %i)", alias.c_str(), physical_dev.c_str(),
                    filesystem_dev.c_str(), perm.c_str(), arg_addr, argSize);
    return 0;
}
...
static u32 sceIoOpen(const char *filename, int flags, int mode)
{
    ...
    if (f == NULL) {
        ...
        if (error == (int)SCE_KERNEL_ERROR_NOCWD) {
            ERROR_LOG(SCEIO, "SCE_KERNEL_ERROR_NOCWD=sceIoOpen(%s, %08x, %08x) - no current working directory",
                      filename, flags, mode);
            ...
            ERROR_LOG(SCEIO, "ERROR_ERRNO_FILE_NOT_FOUND=sceIoOpen(%s, %08x, %08x) - file not found", filename, flags,
                      mode);
...
static u32 sceIoDevctl(const char *name, int cmd, u32 argAddr, int argLen, u32 outPtr, int outLen)
{
    ...
    switch (cmd) {
        ...
        case 0x01F100A8:
        case 0x01F100A9:
            ERROR_LOG_REPORT(SCEIO, "UNIMPL sceIoDevctl(\"%s\", %08x, %08x, %i, %08x, %i)", name, cmd, argAddr, argLen,
                             outPtr, outLen);
            return 0;
    }
    ...
    */
    ERROR_LOG_REPORT(SCEIO, "UNIMPL sceIoDevctl(\"%s\", %08x, %08x, %i, %08x, %i)", name, cmd, argAddr, argLen, outPtr,
                     outLen);
...
static u32 sceIoOpenAsync(const char *filename, int flags, int mode)
{
    ...
    if (f == NULL) {
        ERROR_LOG(SCEIO, "ERROR_ERRNO_FILE_NOT_FOUND=sceIoOpenAsync(%s, %08x, %08x) - file not found", filename, flags,
                  mode);
    ...
    // TODO: Timing is very inconsistent.  From ms0, 10ms - 20ms depending on filesize/dir depth?  From umd, can take >
    // 1s. For now let's aim low.
...
static u32 sceIoGetAsyncStat(int id, u32 poll, u32 address)
{
    ...
    if (f) {
        if (__IsInInterrupt()) {
            DEBUG_LOG(SCEIO, "%lli = sceIoGetAsyncStat(%i, %i, %08x): illegal context", f->asyncResult, id, poll,
                      address);
            return SCE_KERNEL_ERROR_ILLEGAL_CONTEXT;
        }
        if (f->pendingAsyncResult) {
            if (poll) {
                DEBUG_LOG(SCEIO, "%lli = sceIoGetAsyncStat(%i, %i, %08x): not ready", f->asyncResult, id, poll,
                          address);
                return 1;
            } else {
                if (!__KernelIsDispatchEnabled()) {
                    DEBUG_LOG(SCEIO, "%lli = sceIoGetAsyncStat(%i, %i, %08x): dispatch disabled", f->asyncResult, id,
                              poll, address);
                    return SCE_KERNEL_ERROR_CAN_NOT_WAIT;
                }
            ...
            if (!__KernelIsDispatchEnabled()) {
                DEBUG_LOG(SCEIO, "%lli = sceIoGetAsyncStat(%i, %i, %08x): dispatch disabled", f->asyncResult, id, poll,
                          address);
                return SCE_KERNEL_ERROR_CAN_NOT_WAIT;
            }
...
class DirListing : public KernelObject {
  public:
    ...
    static int GetStaticIDType()
    {
        return PPSSPP_KERNEL_TMID_DirList;
    }
    int GetIDType() const override
    {
        return PPSSPP_KERNEL_TMID_DirList;
        ...
        if (isFAT) {
            ...
            if (Memory::IsValidAddress(entry->d_private)) {
                if (sceKernelGetCompiledSdkVersion() <= 0x0307FFFF) {
                    ...
                    strcpy_limit((char *)Memory::GetPointer(entry->d_private + 13), (const char *)entry->d_name,
                                 ARRAY_SIZE(entry->d_name));
                } else {
                    ...
                    // Hm, so currently we don't write the short name at all to d_private? TODO
                    if (size >= 1044) {
                        strcpy_limit((char *)Memory::GetPointer(entry->d_private + 20), (const char *)entry->d_name,
                                     ARRAY_SIZE(entry->d_name));
                    }
                }
            }
...
static int __IoIoctl(u32 id, u32 cmd, u32 indataPtr, u32 inlen, u32 outdataPtr, u32 outlen, int &usec)
{
    ...
    // KD Hearts:
    // 56:46:434 HLE\sceIo.cpp:886 E[HLE]: UNIMPL 0=sceIoIoctrl id: 0000011f, cmd 04100001, indataPtr 08b313d8, inlen
    // 00000010, outdataPtr 00000000, outLen 0 	0000000
    ...
    switch (cmd) {
        ...
        default: {
            ...
            if (result == (int)SCE_KERNEL_ERROR_ERRNO_FUNCTION_NOT_SUPPORTED) {
                ...
                ERROR_LOG(
                    SCEIO,
                    "UNIMPL 0=sceIoIoctl id: %08x, cmd %08x, indataPtr %08x, inlen %08x, outdataPtr %08x, outLen %08x",
                    id, cmd, indataPtr, inlen, outdataPtr, outlen);
            }
            return result;
        } break;
    }
...
static u32 sceIoIoctlAsync(u32 id, u32 cmd, u32 indataPtr, u32 inlen, u32 outdataPtr, u32 outlen)
{
    ...
    if (f) {
        if (f->asyncBusy()) {
            WARN_LOG(SCEIO, "sceIoIoctlAsync(%08x, %08x, %08x, %08x, %08x, %08x): async busy", id, cmd, indataPtr,
                     inlen, outdataPtr, outlen);
            return SCE_KERNEL_ERROR_ASYNC_BUSY;
        }
        DEBUG_LOG(SCEIO, "sceIoIoctlAsync(%08x, %08x, %08x, %08x, %08x, %08x)", id, cmd, indataPtr, inlen, outdataPtr,
                  outlen);
        int usec       = 100;
...
const HLEFunction IoFileMgrForUser[] = {
    ...
    {0x54F5FB11, &WrapU_CIUIUI<sceIoDevctl>,
     "sceIoDevctl"},    //(const char *name int cmd, void *arg, size_t arglen, void *buf, size_t *buflen);
