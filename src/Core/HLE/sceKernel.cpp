    ...
    for (int i = 0; i < maxCount; i++) {
        if (occupied[i]) {
            ...
            if (pool[i]) {
                ...
                INFO_LOG(SCEKERNEL, "KO %i: %s \"%s\": %s", i + handleOffset, pool[i]->GetTypeName(),
                         pool[i]->GetName(), buffer);
            } else {
...
static int ThreadManForKernel_446d8de6(const char *threadName, u32 entry, u32 prio, int stacksize, u32 attr,
                                       u32 optionAddr)
{
...
const HLEFunction ThreadManForUser[] = {
    ...
    {0xBD123D9E, WrapI_U<sceKernelDelaySysClockThread>, "sceKernelDelaySysClockThread",
     HLE_NOT_IN_INTERRUPT | HLE_NOT_DISPATCH_SUSPENDED},
    {0x1181E963, WrapI_U<sceKernelDelaySysClockThreadCB>, "sceKernelDelaySysClockThreadCB",
     HLE_NOT_IN_INTERRUPT | HLE_NOT_DISPATCH_SUSPENDED},
