...
int isPTPPortInUse(uint16_t port)
{
    ...
    int i = 0;
    for (; i < 255; i++)
        if (ptp[i] != NULL && ptp[i]->lport == port)
            return 1;
...
SceNetAdhocctlScanInfo *findGroup(SceNetEtherAddr *MAC)
{
    ...
    // Iterate Groups
    for (; group != NULL; group = group->next) {
        if (IsMatch(group->bssid.mac_addr, *MAC))
            break;
    }
...
void deleteFriendByIP(uint32_t ip)
{
    ...
    for (; peer != NULL; peer = peer->next) {
        // Found Peer
        if (peer->ip_addr == ip) {
            // Instead of removing it from the list we'll make it timeout since most Matching games are moving group and
            // may still need the peer data
            peer->last_recv = 0;
...
void postAcceptCleanPeerList(SceNetAdhocMatchingContext *context)
{
    ...
    while (peer != NULL) {
        ...
        // Unneeded Peer
        if (peer->state != PSP_ADHOC_MATCHING_PEER_CHILD && peer->state != PSP_ADHOC_MATCHING_PEER_P2P &&
...
void postAcceptAddSiblings(SceNetAdhocMatchingContext *context, int siblingcount, SceNetEtherAddr *siblings)
{
    ...
    for (; i < siblingcount; i++) {
        ...
        SceNetAdhocMatchingMemberInternal *sibling =
            (SceNetAdhocMatchingMemberInternal *)malloc(sizeof(SceNetAdhocMatchingMemberInternal));
        ...
        if (sibling != NULL) {
            ...
            INFO_LOG(SCENET, "Accepting Peer %02X:%02X:%02X:%02X:%02X:%02X", sibling->mac.data[0], sibling->mac.data[1],
                     sibling->mac.data[2], sibling->mac.data[3], sibling->mac.data[4], sibling->mac.data[5]);
        }
    }
}
...
void deletePeer(SceNetAdhocMatchingContext *context, SceNetAdhocMatchingMemberInternal *peer)
{
    ...
    if (context != NULL && peer != NULL) {
        ...
        if (item != NULL) {
            ...
            INFO_LOG(SCENET, "Removing Peer %02X:%02X:%02X:%02X:%02X:%02X", peer->mac.data[0], peer->mac.data[1],
                     peer->mac.data[2], peer->mac.data[3], peer->mac.data[4], peer->mac.data[5]);
        }
 ...
 */
void sendGenericMessage(SceNetAdhocMatchingContext *context, int stack, SceNetEtherAddr *mac, int opcode, int optlen,
                        const void *opt)
{
 ...
 * @param opt Optional Data
 */
void sendAcceptMessage(SceNetAdhocMatchingContext *context, SceNetAdhocMatchingMemberInternal *peer, int optlen,
                       const void *opt)
{
    // Send Accept Message
    sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac, PSP_ADHOC_MATCHING_PACKET_ACCEPT, optlen,
                       opt);
}
 ...
 * @param opt Optional Data
 */
void sendJoinRequest(SceNetAdhocMatchingContext *context, SceNetAdhocMatchingMemberInternal *peer, int optlen,
                     const void *opt)
{
    // Send Join Message
    sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac, PSP_ADHOC_MATCHING_PACKET_JOIN, optlen,
                       opt);
}
 ...
 * @param opt Optional Data
 */
void sendCancelMessage(SceNetAdhocMatchingContext *context, SceNetAdhocMatchingMemberInternal *peer, int optlen,
                       const void *opt)
{
    // Send Cancel Message
    sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac, PSP_ADHOC_MATCHING_PACKET_CANCEL, optlen,
                       opt);
}
 ...
 * @param data Data
 */
void sendBulkData(SceNetAdhocMatchingContext *context, SceNetAdhocMatchingMemberInternal *peer, int datalen,
                  const void *data)
{
    // Send Bulk Data Message
    sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac, PSP_ADHOC_MATCHING_PACKET_BULK, datalen,
                       data);
}
...
void abortBulkTransfer(SceNetAdhocMatchingContext *context, SceNetAdhocMatchingMemberInternal *peer)
{
    ...
    sendGenericMessage(context, PSP_ADHOC_MATCHING_INPUT_STACK, &peer->mac, PSP_ADHOC_MATCHING_PACKET_BULK_ABORT, 0,
                       NULL);
}
    ...
    while (peer != NULL) {
        ...
        if ((now - peer->lastping) >= context->timeout) {
            ...
            if ((context->mode == PSP_ADHOC_MATCHING_MODE_CHILD &&
                 (peer->state == PSP_ADHOC_MATCHING_PEER_CHILD || peer->state == PSP_ADHOC_MATCHING_PEER_PARENT)) ||
            ...
            INFO_LOG(SCENET, "TimedOut Peer %02X:%02X:%02X:%02X:%02X:%02X (%lldms)", peer->mac.data[0],
                     peer->mac.data[1], peer->mac.data[2], peer->mac.data[3], peer->mac.data[4], peer->mac.data[5],
...
void AfterMatchingMipsCall::run(MipsCall &call)
{
    if (context == NULL)
        return;
    DEBUG_LOG(SCENET, "Entering AfterMatchingMipsCall::run [ID=%i][Event=%d] [cbId: %u]", context->id, EventID,
              call.cbId);
    // u32 v0 = currentMIPS->r[MIPS_REG_V0];
    if (__IsInInterrupt())
        ERROR_LOG(SCENET, "AfterMatchingMipsCall::run [ID=%i][Event=%d] is Returning Inside an Interrupt!", context->id,
                  EventID);
    ...
    DEBUG_LOG(SCENET, "Leaving AfterMatchingMipsCall::run [ID=%i][Event=%d] [retV0: %08x]", context->id, EventID,
...
// Matching callback is void function: typedef void(*SceNetAdhocMatchingHandler)(int id, int event, SceNetEtherAddr *
// peer, int optlen, void * opt); Important! The MIPS call need to be fully executed before the next MIPS call invoked,
// as the game (ie. DBZ Tag Team) may need to prepare something for the next callback event to use Note: Must not lock
// peerlock within this function to prevent race-condition with other thread whos owning peerlock and trying to lock
// context->eventlock owned by this thread
void notifyMatchingHandler(SceNetAdhocMatchingContext *context, ThreadMessage *msg, void *opt, u32 &bufAddr,
                           u32 &bufLen, u32_le *args)
{
    ...
    if (msg->optlen > 0)
        memcpy(optPtr + 8, opt, msg->optlen);
    args[0] = context->id;
    args[1] = msg->opcode;
    args[2] = bufAddr;    // PSP_GetScratchpadMemoryBase() + 0x6000;
    args[3] = msg->optlen;
    args[4] = args[2] + 8;
    args[5] = context->handler.entryPoint;    // not part of callback argument, just borrowing a space to store callback
                                              // address so i don't need to search the context first later

    context->IsMatchingInCB = true;
    // ScheduleEvent_Threadsafe_Immediate seems to get mixed up with interrupt (returning from mipscall inside an
    // interrupt) and getting invalid address before returning from interrupt
    ...
    // sleep_ms(20); // Wait a little more (for context switching may be?) to prevent DBZ Tag Team from getting
    // connection lost, but this will cause lags on Lord of Arcana
}
...
int friendFinder()
{
    ...
    while (friendFinderRunning) {
        ...
        // Ping Server
        now = real_time_now() *
              1000000.0;    // should be in microseconds, but it seems real_time_now() returns in seconds
        if (now - lastping >= PSP_ADHOCCTL_PING_TIMEOUT) {    // 100 // We need to use lower interval to prevent getting
                                                              // timeout at Pro Adhoc Server through internet
            ...
            // Send Ping to Server, may failed with socket error 10054/10053 if someone else with the same IP already
            // connected to AdHoc Server (the server might need to be modified to differentiate MAC instead of IP)
        ...
        if (rxpos > 0) {
            ...
            // Chat Packet
                ...
                if (rxpos >= (int)sizeof(SceNetAdhocctlChatPacketS2C)) {
                    ...
                    if (strcasecmp((char *)packet->name.data, "ME") == 0)
                        strcpy((char *)packet->name.data, "NOT ME");
            ...
            // Scan Packet
                ...
                if (rxpos >= (int)sizeof(SceNetAdhocctlScanPacketS2C)) {
                    ...
                    // Should only add non-existing group (or replace an existing group) to prevent Ford Street Racing
                    // from showing a strange game session list
                    ...
                    else*/
                    {
                        ...
                        SceNetAdhocctlScanInfo *group =
                            (SceNetAdhocctlScanInfo *)malloc(sizeof(SceNetAdhocctlScanInfo));
        ...
        while (Core_IsStepping() && friendFinderRunning)
            sleep_ms(1);
    }
...
{    // opt = SO_RCVBUF/SO_SNDBUF
    ...
    getsockopt(sock, SOL_SOCKET, opt, (char *)&n,
               &m);    // in linux the value is twice of the value being set using setsockopt
    return (n / 2);
}
...
int initNetwork(SceNetAdhocctlAdhocId *adhoc_id)
{
    ...
    if (iResult == SOCKET_ERROR) {
        ...
        char     buffer[512];
        snprintf(buffer, sizeof(buffer), "Socket error (%i) when connecting to %s/%u.%u.%u.%u:%u", errno,
                 g_Config.proAdhocServer.c_str(), sip[0], sip[1], sip[2], sip[3], ntohs(server_addr.sin_port));
...
{
    // Broadcast MAC
    if (memcmp(addr->data, "\xFF\xFF\xFF\xFF\xFF\xFF", ETHER_ADDR_LEN) == 0)
        return true;
    // Normal MAC
    return false;
}
...
bool validNetworkName(const SceNetAdhocctlGroupName *group_name)
{
    ...
    if (group_name != NULL) {
        ...
        int i = 0;
        for (; i < ADHOCCTL_GROUPNAME_LEN && valid; i++) {
            // End of Name
