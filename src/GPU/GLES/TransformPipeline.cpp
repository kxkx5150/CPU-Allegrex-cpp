...
// into one VBO.
//
// This will likely be a win because I believe that between every change of VBO + glVertexAttribPointer*N, the driver
// will perform a lot of validation, probably at draw call time, while all the validation can be skipped if the only
// thing that changes between two draw calls is simple state or texture or a matrix etc, not anything vertex related.
...
extern const GLuint glprim[8] = {
    GL_POINTS, GL_LINES, GL_LINE_STRIP, GL_TRIANGLES, GL_TRIANGLE_STRIP, GL_TRIANGLE_FAN, GL_TRIANGLES,
...
{
    VAI_KILL_AGE            = 120,
    VAI_UNRELIABLE_KILL_AGE = 240,
    VAI_UNRELIABLE_KILL_MAX = 4
};


TransformDrawEngine::TransformDrawEngine()
    : decodedVerts_(0), prevPrim_(GE_PRIM_INVALID), dec_(0), lastVType_(-1), shaderManager_(0), textureCache_(0),
      framebufferManager_(0), numDrawCalls(0), vertexCountInDrawCalls(0), decodeCounter_(0), dcid_(0), uvScale(0),
      fboTexBound_(false)
{
...
void TransformDrawEngine::SubmitPrim(void *verts, void *inds, GEPrimitiveType prim, int vertexCount, u32 vertType,
                                     int *bytesRead)
{
    ...
    *bytesRead = vertexCount * dec_->VertexSize();
    ...
    if (prim == GE_PRIM_RECTANGLES &&
        (gstate.getTextureAddress(0) & 0x3FFFFFFF) == (gstate.getFrameBufAddress() & 0x3FFFFFFF)) {
...
void TransformDrawEngine::DecodeVertsStep()
{
    ...
    if (indexType == (GE_VTYPE_IDX_NONE >> GE_VTYPE_IDX_SHIFT)) {
        ...
        dec_->DecodeVerts(decoded + decodedVerts_ * (int)dec_->GetDecVtxFmt().stride, dc.verts, indexLowerBound,
                          indexUpperBound);
        ...
        switch (indexType) {
            case GE_VTYPE_IDX_8BIT >> GE_VTYPE_IDX_SHIFT:
                for (int j = i; j <= lastMatch; j++) {
                    indexGen.TranslatePrim(drawCalls[j].prim, drawCalls[j].vertexCount, (const u8 *)drawCalls[j].inds,
                                           indexLowerBound);
                }
                break;
            case GE_VTYPE_IDX_16BIT >> GE_VTYPE_IDX_SHIFT:
                for (int j = i; j <= lastMatch; j++) {
                    indexGen.TranslatePrim(drawCalls[j].prim, drawCalls[j].vertexCount, (const u16 *)drawCalls[j].inds,
                                           indexLowerBound);
                }
                break;
        }
...
u32 TransformDrawEngine::ComputeMiniHash()
{
    ...
    for (int i = 0; i < numDrawCalls; i += step) {
        ...
        if (!dc.inds) {
            ...
            int indexLowerBound = dc.indexLowerBound, indexUpperBound = dc.indexUpperBound;
            fullhash += ComputeMiniHashRange((const u8 *)dc.verts + vertexSize * indexLowerBound,
...
void TransformDrawEngine::DoFlush()
{
    ...
    if (vshader->UseHWTransform()) {
        ...
        int    vertexCount = 0;
        int    maxIndex =
            0;    // Compiler warns about this because it's only used in the #ifdeffed out RangeElements path.
            ...
            switch (vai->status) {
                ...
                case VertexArrayInfo::VAI_HASHING: {
                    ...
                    if (vai->vbo == 0) {
                        ...
                        glBufferData(GL_ARRAY_BUFFER, dec_->GetDecVtxFmt().stride * indexGen.MaxIndex(), decoded,
                                     GL_STATIC_DRAW);
                        ...
                        if (useElements) {
                            ...
                            glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(short) * indexGen.VertexCount(),
                                         (GLvoid *)decIndex, GL_STATIC_DRAW);
                        } else {
                            vai->ebo = 0;
                            glstate.elementArrayBuffer.bind(vai->ebo);
                        }
                    ...
                    // Reliable - we don't even bother hashing anymore. Right now we don't go here until after a very
                    // long time.
        ...
        if (gstate.isModeThrough()) {
            ...
            gstate_c.vertexFullAlpha =
                gstate_c.vertexFullAlpha &&
                ((hasColor && (gstate.materialupdate & 1)) || gstate.getMaterialAmbientA() == 255) &&
...
#else
            glDrawRangeElements(glprim[prim], 0, maxIndex, vertexCount, GL_UNSIGNED_SHORT,
                                ebo ? 0 : (GLvoid *)decIndex);
#endif
        } else {
        ...
        SoftwareTransform(prim, decoded, indexGen.VertexCount(), dec_->VertexType(), inds, GE_VTYPE_IDX_16BIT,
                          dec_->GetDecVtxFmt(), maxIndex, framebufferManager_, textureCache_, transformed,
                          transformedExpanded, drawBuffer, numTrans, drawIndexed, &result);
        ...
        if (result.action == SW_DRAW_PRIMITIVES) {
            ...
            if (attrMask & (1 << ATTR_TEXCOORD))
                glVertexAttribPointer(ATTR_TEXCOORD, doTextureProjection ? 3 : 2, GL_FLOAT, GL_FALSE, vertexSize,
                                      ((uint8_t *)drawBuffer) + offsetof(TransformedVertex, u));
            if (attrMask & (1 << ATTR_COLOR0))
                glVertexAttribPointer(ATTR_COLOR0, 4, GL_UNSIGNED_BYTE, GL_TRUE, vertexSize,
                                      ((uint8_t *)drawBuffer) + offsetof(TransformedVertex, color0));
            if (attrMask & (1 << ATTR_COLOR1))
                glVertexAttribPointer(ATTR_COLOR1, 3, GL_UNSIGNED_BYTE, GL_TRUE, vertexSize,
                                      ((uint8_t *)drawBuffer) + offsetof(TransformedVertex, color1));
            if (drawIndexed) {
#if 1    // USING_GLES2
