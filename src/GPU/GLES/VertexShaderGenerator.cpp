...
void GenerateVertexShader(int prim, u32 vertType, char *buffer, bool useHWTransform)
{
    ...
    bool prescale = g_Config.bPrescaleUV && !throughmode &&
                    (gstate.getUVGenMode() == GE_TEXMAP_TEXTURE_COORDS || gstate.getUVGenMode() == GE_TEXMAP_UNKNOWN);
    ...
    if (useHWTransform) {
        ...
        if (doTexture && (flipV || !prescale || gstate.getUVGenMode() == GE_TEXMAP_ENVIRONMENT_MAP ||
                          gstate.getUVGenMode() == GE_TEXMAP_TEXTURE_MATRIX)) {
    ...
    if (!useHWTransform) {
        ...
        if (!vertTypeIsSkinningEnabled(vertType)) {
            ...
            static const char *rescale[4] = {"", " * 1.9921875", " * 1.999969482421875",
                                             ""};    // 2*127.5f/128.f, 2*32767.5f/32768.f, 1.0f};
            const char        *factor     = rescale[vertTypeGetWeightMask(vertType) >> GE_VTYPE_WEIGHT_SHIFT];

            static const char *const boneWeightAttr[8] = {
                "w1.x", "w1.y", "w1.z", "w1.w", "w2.x", "w2.y", "w2.z", "w2.w",
            };
            ...
            switch (numWeights) {
                case 1:
                    WRITE(p, "  float w[1]; w[0] = w1;\n");
                    break;
                case 2:
                    WRITE(p, "  float w[2]; w[0] = w1.x; w[1] = w1.y;\n");
                    break;
                case 3:
                    WRITE(p, "  float w[3]; w[0] = w1.x; w[1] = w1.y; w[2] = w1.z;\n");
                    break;
                case 4:
                    WRITE(p, "  float w[4]; w[0] = w1.x; w[1] = w1.y; w[2] = w1.z; w[3] = w1.w;\n");
                    break;
                case 5:
                    WRITE(p, "  float w[5]; w[0] = w1.x; w[1] = w1.y; w[2] = w1.z; w[3] = w1.w; w[4] = w2;\n");
                    break;
                case 6:
                    WRITE(p, "  float w[6]; w[0] = w1.x; w[1] = w1.y; w[2] = w1.z; w[3] = w1.w; w[4] = w2.x; w[5] = "
                             "w2.y;\n");
                    break;
                case 7:
                    WRITE(p, "  float w[7]; w[0] = w1.x; w[1] = w1.y; w[2] = w1.z; w[3] = w1.w; w[4] = w2.x; w[5] = "
                             "w2.y; w[6] = w2.z;\n");
                    break;
                case 8:
                    WRITE(p, "  float w[8]; w[0] = w1.x; w[1] = w1.y; w[2] = w1.z; w[3] = w1.w; w[4] = w2.x; w[5] = "
                             "w2.y; w[6] = w2.z; w[7] = w2.w;\n");
                    break;
            }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            switch (type) {
                ...
                case GE_LIGHTTYPE_POINT:
                    WRITE(p,
                          "  lightScale = clamp(1.0 / dot(u_lightatt%i, vec3(1.0, distance, distance*distance)), 0.0, "
                          "1.0);\n",
                          i);
                    break;
                ...
                case GE_LIGHTTYPE_UNKNOWN:
                    ...
                    WRITE(p,
                          "    lightScale = clamp(1.0 / dot(u_lightatt%i, vec3(1.0, distance, distance*distance)), "
                          "0.0, 1.0) * pow(angle%i, u_lightspotCoef%i);\n",
                          i, i, i);
                    WRITE(p, "  } else {\n");
        ...
        if (doTexture) {
            switch (gstate.getUVGenMode()) {
                ...
                case GE_TEXMAP_TEXTURE_MATRIX:    // Projection mapping.
                {
                    ...
                    switch (gstate.getUVProjMode()) {
                        ...
                        case GE_PROJMAP_UV:    // Use unscaled UV as source
                        {
                            ...
                            if (hasTexcoord) {
                                static const char *rescaleuv[4] = {"", " * 1.9921875", " * 1.999969482421875",
                                                                   ""};    // 2*127.5f/128.f, 2*32767.5f/32768.f, 1.0f};
                ...
                case GE_TEXMAP_ENVIRONMENT_MAP:    // Shade mapping - use dots from light sources.
                    WRITE(p,
                          "  v_texcoord = u_uvscaleoffset.xy * vec2(1.0 + dot(normalize(u_lightpos%i), worldnormal), "
                          "1.0 + dot(normalize(u_lightpos%i), worldnormal)) * 0.5;\n",
