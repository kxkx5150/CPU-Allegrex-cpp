...
static const CommandTableEntry commandTable[] = {
    ...
    {GE_CMD_TEXMAPMODE, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_UVSCALEOFFSET,
     &GLES_GPU::Execute_TexMapMode},
    {GE_CMD_TEXSCALEU, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_UVSCALEOFFSET,
     &GLES_GPU::Execute_TexScaleU},
    {GE_CMD_TEXSCALEV, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_UVSCALEOFFSET,
     &GLES_GPU::Execute_TexScaleV},
    {GE_CMD_TEXOFFSETU, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_UVSCALEOFFSET,
     &GLES_GPU::Execute_TexOffsetU},
    {GE_CMD_TEXOFFSETV, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_UVSCALEOFFSET,
     &GLES_GPU::Execute_TexOffsetV},
    ...
    {GE_CMD_COLORTESTMASK, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_ALPHACOLORMASK,
     &GLES_GPU::Execute_ColorTestMask},
    ...
    // Uniform changes
    {GE_CMD_ALPHATEST, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_ALPHACOLORREF | DIRTY_ALPHACOLORMASK,
     &GLES_GPU::Execute_AlphaTest},
    ...
    {GE_CMD_STENCILTEST, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_STENCILREPLACEVALUE,
     &GLES_GPU::Execute_StencilTest},
    ...
    // These dirty various vertex shader uniforms. Could embed information about that in this table and call
    // dirtyuniform directly, hm...
    ...
    {GE_CMD_MATERIALDIFFUSE, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_MATDIFFUSE,
     &GLES_GPU::Execute_MaterialDiffuse},
    {GE_CMD_MATERIALEMISSIVE, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_MATEMISSIVE,
     &GLES_GPU::Execute_MaterialEmissive},
    {GE_CMD_MATERIALAMBIENT, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_MATAMBIENTALPHA,
     &GLES_GPU::Execute_MaterialAmbient},
    {GE_CMD_MATERIALALPHA, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_MATAMBIENTALPHA,
     &GLES_GPU::Execute_MaterialAmbient},
    {GE_CMD_MATERIALSPECULAR, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_MATSPECULAR,
     &GLES_GPU::Execute_MaterialSpecular},
    {GE_CMD_MATERIALSPECULARCOEF, FLAG_FLUSHBEFOREONCHANGE | FLAG_EXECUTEONCHANGE, DIRTY_MATSPECULAR,
     &GLES_GPU::Execute_MaterialSpecular},
    ...
    {GE_CMD_END, FLAG_FLUSHBEFORE | FLAG_EXECUTE | FLAG_READS_PC | FLAG_WRITES_PC, 0,
     &GPUCommon::Execute_End},    // Flush?
    ...
    {GE_CMD_BOUNDINGBOX, FLAG_EXECUTE, 0,
     &GLES_GPU::Execute_BoundingBox},    // + FLUSHBEFORE when we implement... or not, do we need to?
...
GLES_GPU::GLES_GPU() : resized_(false)
{
...
void GLES_GPU::BuildReportingInfo()
{
    ...
    char temp[16384];
    snprintf(temp, sizeof(temp), "%s (%s %s), %s (extensions: %s)", glVersion, glVendor, glRenderer, glSlVersion,
             glExtensions);
...
inline void GLES_GPU::UpdateVsyncInterval(bool force)
{
    ...
    if (PSP_CoreParameter().fpsLimit == 1) {
        ...
        if (g_Config.iFpsLimit == 0 ||
            (g_Config.iFpsLimit != 15 && g_Config.iFpsLimit != 30 && g_Config.iFpsLimit != 60)) {
            desiredVSyncInterval = 0;
        }
    }
...
void GLES_GPU::FastRunLoop(DisplayList &list)
{
    ...
    for (; dc > 0; --dc) {
        ...
        const u8 cmdFlags = info.flags;    // If we stashed the cmdFlags in the top bits of the cmdmem, we could get
                                           // away with one table lookup instead of two
...
void GLES_GPU::Execute_Bezier(u32 op, u32 diff)
{
    ...
    if (gstate.vertType & GE_VTYPE_MORPHCOUNT_MASK) {
        DEBUG_LOG_REPORT(G3D, "Bezier + morph: %i",
                         (gstate.vertType & GE_VTYPE_MORPHCOUNT_MASK) >> GE_VTYPE_MORPHCOUNT_SHIFT);
    }
...
void GLES_GPU::Execute_Spline(u32 op, u32 diff)
{
    ...
    if (gstate.vertType & GE_VTYPE_MORPHCOUNT_MASK) {
        DEBUG_LOG_REPORT(G3D, "Spline + morph: %i",
                         (gstate.vertType & GE_VTYPE_MORPHCOUNT_MASK) >> GE_VTYPE_MORPHCOUNT_SHIFT);
    }
    if (vertTypeIsSkinningEnabled(gstate.vertType)) {
    ...
    GEPatchPrimType patchPrim = gstate.getPatchPrimitiveType();
    transformDraw_.SubmitSpline(control_points, indices, sp_ucount, sp_vcount, sp_utype, sp_vtype, patchPrim,
                                gstate.vertType);
}
...
void GLES_GPU::DoBlockTransfer()
{
    ...
    DEBUG_LOG(G3D, "Block transfer: %08x/%x -> %08x/%x, %ix%ix%i (%i,%i)->(%i,%i)", srcBasePtr, srcStride, dstBasePtr,
              dstStride, width, height, bpp, srcX, srcY, dstX, dstY);
    ...
    if (!Memory::IsValidAddress(srcLastAddr)) {
        ERROR_LOG_REPORT(G3D, "Bottom-right corner of source of block transfer is at an invalid address: %08x",
                         srcLastAddr);
        return;
    }
    if (!Memory::IsValidAddress(dstLastAddr)) {
        ERROR_LOG_REPORT(G3D, "Bottom-right corner of destination of block transfer is at an invalid address: %08x",
                         srcLastAddr);
        return;
    }

    // Tell the framebuffer manager to take action if possible. If it does the entire thing, let's just return.
    if (!framebufferManager_.NotifyBlockTransferBefore(dstBasePtr, dstStride, dstX, dstY, srcBasePtr, srcStride, srcX,
                                                       srcY, width, height, bpp)) {
        ...
        textureCache_.Invalidate(dstBasePtr + (dstY * dstStride + dstX) * bpp, height * dstStride * bpp,
                                 GPU_INVALIDATE_HINT);
        framebufferManager_.NotifyBlockTransferAfter(dstBasePtr, dstStride, dstX, dstY, srcBasePtr, srcStride, srcX,
                                                     srcY, width, height, bpp);
    }

#ifndef MOBILE_DEVICE
    CBreakPoints::ExecMemCheck(srcBasePtr + (srcY * srcStride + srcX) * bpp, false, height * srcStride * bpp,
                               currentMIPS->pc);
    CBreakPoints::ExecMemCheck(dstBasePtr + (srcY * dstStride + srcX) * bpp, true, height * dstStride * bpp,
                               currentMIPS->pc);
#endif
