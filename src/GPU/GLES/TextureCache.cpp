...
TextureCache::TextureCache()
    : cacheSizeEstimate_(0), secondCacheSizeEstimate_(0), clearCacheNextFrame_(false), lowMemoryMode_(false),
      clutBuf_(NULL), clutMaxBytes_(0), texelsScaledThisFrame_(0)
{
...
void TextureCache::Decimate()
{
    ...
    if (cacheSizeEstimate_ >= TEXCACHE_MIN_PRESSURE) {
        ...
        VERBOSE_LOG(G3D, "Decimated texture cache, saved %d estimated bytes - now %d bytes", had - cacheSizeEstimate_,
                    cacheSizeEstimate_);
    }
    ...
    if (g_Config.bTextureSecondaryCache && secondCacheSizeEstimate_ >= TEXCACHE_SECOND_MIN_PRESSURE) {
        ...
        VERBOSE_LOG(G3D, "Decimated second texture cache, saved %d estimated bytes - now %d bytes",
                    had - secondCacheSizeEstimate_, secondCacheSizeEstimate_);
    }
}
...
void TextureCache::AttachFramebufferValid(TexCacheEntry *entry, VirtualFramebuffer *framebuffer,
                                          const AttachedFramebufferInfo &fbInfo)
{
    const bool hasInvalidFramebuffer = entry->framebuffer == nullptr || entry->invalidHint == -1;
    const bool hasOlderFramebuffer =
        entry->framebuffer != nullptr && entry->framebuffer->last_frame_render < framebuffer->last_frame_render;
...
void TextureCache::AttachFramebufferInvalid(TexCacheEntry *entry, VirtualFramebuffer *framebuffer,
                                            const AttachedFramebufferInfo &fbInfo)
{
...
bool TextureCache::AttachFramebuffer(TexCacheEntry *entry, u32 address, VirtualFramebuffer *framebuffer,
                                     u32 texaddrOffset)
{
    ...
    if (exactMatch) {
        ...
        if (framebuffer->fb_stride != entry->bufw) {
            WARN_LOG_REPORT_ONCE(diffStrides1, G3D, "Render to texture with different strides %d != %d", entry->bufw,
                                 framebuffer->fb_stride);
        }
        if (entry->format != (GETextureFormat)framebuffer->format) {
            WARN_LOG_REPORT_ONCE(diffFormat1, G3D, "Render to texture with different formats %d != %d", entry->format,
                                 framebuffer->format);
        ...
        const bool clutFormat = (framebuffer->format == GE_FORMAT_8888 && entry->format == GE_TFMT_CLUT32) ||
        ...
        if (framebuffer->fb_stride != entry->bufw) {
            if (noOffset) {
                WARN_LOG_REPORT_ONCE(diffStrides2, G3D, "Render to texture using CLUT with different strides %d != %d",
                                     entry->bufw, framebuffer->fb_stride);
            } else {
        ...
        if (fbInfo.yOffset > MAX_SUBAREA_Y_OFFSET_SAFE && addr > 0x04110000) {
            WARN_LOG_REPORT_ONCE(subareaIgnored, G3D, "Ignoring possible render to texture at %08x +%dx%d / %dx%d",
                                 address, fbInfo.xOffset, fbInfo.yOffset, framebuffer->width, framebuffer->height);
            DetachFramebuffer(entry, address, framebuffer);
            return false;
        }
        ...
        if (clutFormat) {
            if (!noOffset) {
                WARN_LOG_REPORT_ONCE(subareaClut, G3D, "Render to texture using CLUT with offset at %08x +%dx%d",
                                     address, fbInfo.xOffset, fbInfo.yOffset);
            }
            AttachFramebufferValid(entry, framebuffer, fbInfo);
        ...
        if (framebuffer->format == entry->format || clutFormat) {
            if (framebuffer->format != entry->format) {
                WARN_LOG_REPORT_ONCE(diffFormat2, G3D, "Render to texture with different formats %d != %d at %08x",
                                     entry->format, framebuffer->format, address);
                AttachFramebufferValid(entry, framebuffer, fbInfo);
                return true;
            } else {
                WARN_LOG_REPORT_ONCE(subarea, G3D, "Render to area containing texture at %08x +%dx%d", address,
                                     fbInfo.xOffset, fbInfo.yOffset);
                // If "AttachFramebufferValid" ,  God of War Ghost of Sparta/Chains of Olympus will be missing special
                // effect.
                AttachFramebufferInvalid(entry, framebuffer, fbInfo);
                return true;
            }
        } else {
            WARN_LOG_REPORT_ONCE(diffFormat2, G3D, "Render to texture with incompatible formats %d != %d at %08x",
                                 entry->format, framebuffer->format, address);
        }
    }
...
void TextureCache::NotifyFramebuffer(u32 address, VirtualFramebuffer *framebuffer, FramebufferNotification msg)
{
    ...
    switch (msg) {
        ...
        case NOTIFY_FB_UPDATED:
            ...
            for (auto it = cache.lower_bound(mirrorCacheKey), end = cache.upper_bound(mirrorCacheKeyEnd); it != end;
                 ++it) {
                AttachFramebuffer(&it->second, addr, framebuffer);
            }
            break;
...
static const GLuint MagFiltGL[2] = {GL_NEAREST, GL_LINEAR};

void TextureCache::GetSamplingParams(int &minFilt, int &magFilt, bool &sClamp, bool &tClamp, float &lodBias,
                                     int maxLevel)
{
    ...
    bool noMip =
        (gstate.texlevel & 0xFFFFFF) == 0x000001 || (gstate.texlevel & 0xFFFFFF) == 0x100001;    // Fix texlevel at 0
    ...
    if ((gstate.isColorTestEnabled() && !IsColorTestTriviallyTrue()) && g_Config.iInternalResolution != 1 &&
        gstate.isModeThrough()) {
    ...
    switch (dstFmt) {
        case GL_UNSIGNED_SHORT_4_4_4_4: {
            ...
            for (; i < (numPixels + 1) / 2; i++) {
                u32 c  = src[i];
                dst[i] = ((c >> 12) & 0x000F000F) | ((c >> 4) & 0x00F000F0) | ((c << 4) & 0x0F000F00) |
                         ((c << 12) & 0xF000F000);
            }
        } break;
        ...
        case GL_UNSIGNED_SHORT_5_5_5_1: {
            ...
            for (; i < (numPixels + 1) / 2; i++) {
                u32 c  = src[i];
                dst[i] = ((c >> 15) & 0x00010001) | ((c >> 9) & 0x003E003E) | ((c << 1) & 0x07C007C0) |
                         ((c << 11) & 0xF800F800);
            }
        } break;
        case GL_UNSIGNED_SHORT_5_6_5: {
            ...
            for (; i < (numPixels + 1) / 2; i++) {
                u32 c  = src[i];
                dst[i] = ((c >> 11) & 0x001F001F) | ((c >> 0) & 0x07E007E0) | ((c << 11) & 0xF800F800);
            }
        } break;
        default:
            if (UseBGRA8888()) {
                ...
                for (; i < numPixels; i++) {
                    u32 c  = src[i];
                    dst[i] = ((c >> 16) & 0x000000FF) | ((c >> 0) & 0xFF00FF00) | ((c << 16) & 0x00FF0000);
                }
            } else {
...
void TextureCache::UpdateCurrentClut()
{
    ...
    if (UseBGRA8888() || clutFormat != GE_CMODE_32BIT_ABGR8888) {
        const int numColors =
            (clutMaxBytes_ + clutBaseBytes) / (clutFormat == GE_CMODE_32BIT_ABGR8888 ? sizeof(u32) : sizeof(u16));
...
template <typename T> inline const T *TextureCache::GetCurrentClut()
{
...
void TextureCache::SetTextureFramebuffer(TexCacheEntry *entry, VirtualFramebuffer *framebuffer)
{
    ...
    if (useBufferedRendering) {
        ...
        if (program) {
            ...
            FBO   *depalFBO =
                framebufferManager_->GetTempFBO(framebuffer->renderWidth, framebuffer->renderHeight, FBO_8888);
            ...
            TexCacheEntry::Status alphaStatus = CheckAlpha(clutBuf_, getClutDestFormat(gstate.getClutPaletteFormat()),
                                                           clutExtendedColors, clutExtendedColors, 1);
        ...
        gstate_c.needShaderTexClamp = gstate_c.curTextureWidth != (u32)gstate.getTextureWidth(0) ||
                                      gstate_c.curTextureHeight != (u32)gstate.getTextureHeight(0);
...
void TextureCache::SetTexture(bool force)
{
    ...
    if (iter != cache.end()) {
        ...
        if (match) {
            ...
            if (rehash && entry->GetHashStatus() != TexCacheEntry::STATUS_RELIABLE) {
                ...
                if (fullhash != entry->fullhash) {
                    hashFail = true;
                } else if (entry->GetHashStatus() != TexCacheEntry::STATUS_HASHING &&
                           entry->numFrames > TexCacheEntry::FRAMES_REGAIN_TRUST) {
        ...
        if (match && (entry->status & TexCacheEntry::STATUS_TO_SCALE) && g_Config.iTexScalingLevel != 1 &&
            texelsScaledThisFrame_ < TEXCACHE_MAX_TEXELS_SCALED) {
        ...
        if (match) {
            ...
            if (doDelete) {
                if (entry->maxLevel == maxLevel && entry->dim == gstate.getTextureDimension(0) &&
                    entry->format == format && g_Config.iTexScalingLevel == 1) {
    ...
    if (false && gl_extensions.GLES3 && maxLevel > 0) {
        ...
        switch (dstFmt) {
            case GL_UNSIGNED_BYTE:
                ...
                break;
            case GL_UNSIGNED_SHORT_5_6_5:
                storageFmt = GL_RGB565;
                break;
            case GL_UNSIGNED_SHORT_4_4_4_4:
                storageFmt = GL_RGBA4;
                break;
            case GL_UNSIGNED_SHORT_5_5_5_1:
                storageFmt = GL_RGB5_A1;
                break;
            default:
                ERROR_LOG(G3D, "Unknown dstfmt %i", (int)dstFmt);
                break;
        }
        // TODO: This may cause bugs, since it hard-sets the texture w/h, and we might try to reuse it later with a
        // different size.
    ...
    if (maxLevel > 0 && g_Config.iTexScalingLevel == 1) {
        ...
        if (w > 1 && h > 1 &&
            !(gl_extensions.gpuVendor == GPU_VENDOR_POWERVR &&
              h > w)) {    // Really! only seems to fail if height > width
...
void *TextureCache::DecodeTextureLevel(GETextureFormat format, GEPaletteFormat clutformat, int level, u32 &texByteAlign,
                                       GLenum dstFmt, int *bufwout)
{
    void *finalBuf = NULL;
    ...
    if (texaddr & 0x00600000 && Memory::IsVRAMAddress(texaddr)) {
        ...
        WARN_LOG_REPORT_ONCE(texmirror, G3D, "Decoding texture from VRAM mirror at %08x swizzle=%d", texaddr,
                             gstate.isTextureSwizzled() ? 1 : 0);
    }
            ...
            switch (clutformat) {
                ...
                case GE_CMODE_16BIT_ABGR4444: {
                    ...
                    if (!gstate.isTextureSwizzled()) {
                        ...
                        if (clutAlphaLinear_ && mipmapShareClut) {
                            DeIndexTexture4Optimal(tmpTexBuf16.data(), (const u8 *)tmpTexBuf32.data(), bufw * h,
                                                   clutAlphaLinearColor_);
                        } else {
...
TextureCache::TexCacheEntry::Status TextureCache::CheckAlpha(const u32 *pixelData, GLenum dstFmt, int stride, int w,
                                                             int h)
{
        ...
        default:
            for (int y = 0; y < h; y++)
                for (int x = 0; x < bufw; x++) {
                    ...
                    ((u32 *)output)[y * w + x] = ((val & 0xFF000000)) | ((val & 0x00FF0000) >> 16) |
