...
void ConvertFromRGBA8888(u8 *dst, const u8 *src, u32 dstStride, u32 srcStride, u32 width, u32 height,
...
void FramebufferManager::CompileDraw2DProgram()
{
    if (!draw2dprogram_) {
        ...
        if (shaderInfo) {
            ...
            postShaderProgram_ =
                glsl_create(shaderInfo->vertexShaderFile.c_str(), shaderInfo->fragmentShaderFile.c_str(), &errorString);
            if (!postShaderProgram_) {
                ...
                ERROR_LOG(G3D, "Failed to build post-processing program from %s and %s!\n%s",
                          shaderInfo->vertexShaderFile.c_str(), shaderInfo->fragmentShaderFile.c_str(),
                          errorString.c_str());
                ...
                if (postShaderAtOutputResolution_) {
                    float x, y, w, h;
                    CenterRect(&x, &y, &w, &h, 480.0f, 272.0f, (float)PSP_CoreParameter().pixelWidth,
                               (float)PSP_CoreParameter().pixelHeight);
...
FramebufferManager::FramebufferManager()
    : drawPixelsTex_(0), drawPixelsTexFormat_(GE_FORMAT_INVALID), convBuf_(0), draw2dprogram_(0), postShaderProgram_(0),
      stencilUploadProgram_(0), plainColorLoc_(-1), timeLoc_(-1), textureCache_(0), shaderManager_(0),
      usePostShader_(false), postShaderAtOutputResolution_(false), resized_(false), gameUsesSequentialCopies_(false)
#ifndef USING_GLES2
      ,
      pixelBufObj_(0), currentPBO_(0)
#endif
{
}
...
void FramebufferManager::MakePixelTexture(const u8 *srcPixels, GEBufferFormat srcPixelFormat, int srcStride, int width,
                                          int height)
{
    if (drawPixelsTex_ &&
        (drawPixelsTexFormat_ != srcPixelFormat || drawPixelsTexW_ != width || drawPixelsTexH_ != height)) {
        glDeleteTextures(1, &drawPixelsTex_);
        drawPixelsTex_ = 0;
    }
    ...
    glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, width, height, GL_RGBA, GL_UNSIGNED_BYTE,
                    useConvBuf ? convBuf_ : srcPixels);
}

void FramebufferManager::DrawPixels(VirtualFramebuffer *vfb, int dstX, int dstY, const u8 *srcPixels,
                                    GEBufferFormat srcPixelFormat, int srcStride, int width, int height)
{
...
void FramebufferManager::DrawFramebuffer(const u8 *srcPixels, GEBufferFormat srcPixelFormat, int srcStride,
                                         bool applyPostShader)
{
    ...
    // This might draw directly at the backbuffer (if so, applyPostShader is set) so if there's a post shader, we need
    // to apply it here. Should try to unify this path with the regular path somehow, but this simple solution works for
    // most of the post shaders (it always runs at output resolution so FXAA may look odd).
    float x, y, w, h;
    CenterRect(&x, &y, &w, &h, 480.0f, 272.0f, (float)PSP_CoreParameter().pixelWidth,
               (float)PSP_CoreParameter().pixelHeight);
    if (cardboardSettings.enabled) {
        // Left Eye Image
        glstate.viewport.set(cardboardSettings.leftEyeXPosition, cardboardSettings.screenYPosition,
                             cardboardSettings.screenWidth, cardboardSettings.screenHeight);
        if (applyPostShader && usePostShader_ && useBufferedRendering_) {
            DrawActiveTexture(0, x, y, w, h, (float)PSP_CoreParameter().pixelWidth,
                              (float)PSP_CoreParameter().pixelHeight, false, 0.0f, 0.0f, 480.0f / 512.0f, 1.0f,
                              postShaderProgram_);
        } else {
            DrawActiveTexture(0, x, y, w, h, (float)PSP_CoreParameter().pixelWidth,
                              (float)PSP_CoreParameter().pixelHeight, false, 0.0f, 0.0f, 480.0f / 512.0f);
        }

        // Right Eye Image
        glstate.viewport.set(cardboardSettings.rightEyeXPosition, cardboardSettings.screenYPosition,
                             cardboardSettings.screenWidth, cardboardSettings.screenHeight);
        if (applyPostShader && usePostShader_ && useBufferedRendering_) {
            DrawActiveTexture(0, x, y, w, h, (float)PSP_CoreParameter().pixelWidth,
                              (float)PSP_CoreParameter().pixelHeight, false, 0.0f, 0.0f, 480.0f / 512.0f, 1.0f,
...
void FramebufferManager::DrawActiveTexture(GLuint texture, float x, float y, float w, float h, float destW, float destH,
                                           bool flip, float u0, float v0, float u1, float v1, GLSLProgram *program)
{
    if (flip) {
    ...
    if (texture) {
        ...
        if (false && gl_extensions.NV_draw_texture && !program) {
            ...
            glDrawTextureNV(texture, 0, x, y, w, h, 0.0f, u0, v1, u1, v0);
            return;
        }
#endif
    ...
    float pos[12] = {x, y, 0, x + w, y, 0, x + w, y + h, 0, x, y + h, 0};
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                    g_Config.iBufFilter == SCALE_NEAREST ? GL_NEAREST : GL_LINEAR);
    ...
    if (old.fbo) {
        ...
        if (vfb->fbo) {
            ...
            if (!g_Config.bDisableSlowFramebufEffects) {
                BlitFramebuffer(vfb, 0, 0, &old, 0, 0, std::min(vfb->bufferWidth, vfb->width),
                                std::min(vfb->height, vfb->bufferHeight), 0);
            }
        }
        fbo_destroy(old.fbo);
        if (vfb->fbo) {
...
void FramebufferManager::BlitFramebufferDepth(VirtualFramebuffer *src, VirtualFramebuffer *dst)
{
    ...
    if (src->z_address == dst->z_address && src->z_stride != 0 && dst->z_stride != 0 &&
        src->renderWidth == dst->renderWidth && src->renderHeight == dst->renderHeight) {
...
#if defined(USING_GLES2) &&                                                                                            \
    defined(ANDROID)    // We only support this extension on Android, it's not even available on PC.
                if (useNV) {
                    glBlitFramebufferNV(0, 0, src->renderWidth, src->renderHeight, 0, 0, dst->renderWidth,
                                        dst->renderHeight, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
                } else
#endif    // defined(USING_GLES2) && defined(ANDROID)
                    glBlitFramebuffer(0, 0, src->renderWidth, src->renderHeight, 0, 0, dst->renderWidth,
                                      dst->renderHeight, GL_DEPTH_BUFFER_BIT, GL_NEAREST);
...
void FramebufferManager::BindFramebufferColor(int stage, VirtualFramebuffer *framebuffer, bool skipCopy)
{
    ...
    if (!skipCopy && currentRenderVfb_ && framebuffer->fb_address == gstate.getFrameBufRawAddress()) {
        ...
        FBO *renderCopy =
            GetTempFBO(framebuffer->renderWidth, framebuffer->renderHeight, (FBOColorDepth)framebuffer->colorDepth);
        if (renderCopy) {
            VirtualFramebuffer copyInfo = *framebuffer;
            copyInfo.fbo                = renderCopy;
            BlitFramebuffer(&copyInfo, 0, 0, framebuffer, 0, 0, framebuffer->drawnWidth, framebuffer->drawnHeight, 0,
                            false);
...
void FramebufferManager::CopyDisplayToOutput()
{
    ...
    if (!vfb) {
        ...
        if (vfb) {
            ...
            INFO_LOG_REPORT_ONCE(displayoffset, HLE, "Rendering from framebuf with offset %08x -> %08x+%dx%d", addr,
                                 vfb->fb_address, offsetX, offsetY);
        }
    }
    ...
    if (vfb->fbo) {
        ...
        if (!usePostShader_) {
            if (cardboardSettings.enabled) {
                ...
                glstate.viewport.set(cardboardSettings.leftEyeXPosition, cardboardSettings.screenYPosition,
                                     cardboardSettings.screenWidth, cardboardSettings.screenHeight);
                DrawActiveTexture(colorTexture, x, y, w, h, (float)PSP_CoreParameter().pixelWidth,
                                  (float)PSP_CoreParameter().pixelHeight, true, u0, v0, u1, v1);
            ...
            DrawActiveTexture(colorTexture, 0, 0, fbo_w, fbo_h, fbo_w, fbo_h, true, 0.0f, 0.0f, 1.0f, 1.0f,
                              postShaderProgram_);
            ...
            if (g_Config.bEnableCardboard) {
                ...
                glstate.viewport.set(0, 0, PSP_CoreParameter().pixelWidth, PSP_CoreParameter().pixelHeight);
                DrawActiveTexture(colorTexture, x, y, w, h, (float)PSP_CoreParameter().pixelWidth,
                                  (float)PSP_CoreParameter().pixelHeight, true, u0, v0, u1, v1, postShaderProgram_);
            }
        }
...
void FramebufferManager::BlitFramebuffer(VirtualFramebuffer *dst, int dstX, int dstY, VirtualFramebuffer *src, int srcX,
                                         int srcY, int w, int h, int bpp, bool flip)
{
...
#if defined(USING_GLES2) &&                                                                                            \
    defined(ANDROID)    // We only support this extension on Android, it's not even available on PC.
            glBlitFramebufferNV(srcX1, srcY1, srcX2, srcY2, dstX1, dstY1, dstX2, dstY2, GL_COLOR_BUFFER_BIT,
                                GL_NEAREST);
#endif    // defined(USING_GLES2) && defined(ANDROID)
        }
        ...
        DrawActiveTexture(0, dstX1, dstY, w * dstXFactor, h, dst->bufferWidth, dst->bufferHeight, !flip, srcX1 / srcW,
                          srcY / srcH, srcX2 / srcW, (srcY + h) / srcH, draw2dprogram_);
        glBindTexture(GL_TEXTURE_2D, 0);
        textureCache_->ForgetLastTexture();
        glstate.viewport.restore();
    }
...
void FramebufferManager::PackFramebufferAsync_(VirtualFramebuffer *vfb)
{
    ...
    if (pbo.reading) {
        ...
        if (packed) {
            DEBUG_LOG(SCEGE,
                      "Reading PBO to memory , bufSize = %u, packed = %p, fb_address = %08x, stride = %u, pbo = %u",
    ...
    if (vfb) {
        ...
        bool reverseOrder =
            (gl_extensions.gpuVendor == GPU_VENDOR_NVIDIA) || (gl_extensions.gpuVendor == GPU_VENDOR_AMD);
        switch (vfb->format) {
...
void FramebufferManager::PackFramebufferSync_(VirtualFramebuffer *vfb, int x, int y, int w, int h)
{
    ...
    if (packed) {
        DEBUG_LOG(SCEGE, "Reading framebuffer to mem, bufSize = %u, packed = %p, fb_address = %08x", (u32)bufSize,
                  packed, fb_address);
        ...
        if (convert) {
            ...
            ConvertFromRGBA8888(Memory::GetPointer(fb_address + dstByteOffset), packed + byteOffset, vfb->fb_stride,
                                vfb->fb_stride, vfb->width, h, vfb->format);
        }
    }
...
void FramebufferManager::DecimateFBOs()
{
    ...
    for (size_t i = 0; i < vfbs_.size(); ++i) {
        ...
        if (age > FBO_OLD_AGE) {
            INFO_LOG(SCEGE, "Decimating FBO for %08x (%i x %i x %i), age %i", vfb->fb_address, vfb->width, vfb->height,
                     vfb->format, age);
            DestroyFramebuf(vfb);
...
void FramebufferManager::DestroyAllFBOs()
{
    ...
    for (size_t i = 0; i < vfbs_.size(); ++i) {
        VirtualFramebuffer *vfb = vfbs_[i];
        INFO_LOG(SCEGE, "Destroying FBO for %08x : %i x %i x %i", vfb->fb_address, vfb->width, vfb->height,
                 vfb->format);
        DestroyFramebuf(vfb);
    }
    vfbs_.clear();
...
bool FramebufferManager::GetDisplayFramebuffer(GPUDebugBuffer &buffer)
{
    ...
    glReadPixels(0, 0, PSP_CoreParameter().pixelWidth, PSP_CoreParameter().pixelHeight, GL_RGB, GL_UNSIGNED_BYTE,
                 buffer.GetData());
    return true;
}
