...
GPUCommon::GPUCommon() : dumpNextFrame_(false), dumpThisFrame_(false)
{
    Reinitialize();
...
bool GPUCommon::InterpretList(DisplayList &list)
{
    ...
    const bool useDebugger    = false;    // host->GPUDebuggingActive();
        ...
        case GE_CMD_SIGNAL: {
            ...
            switch (behaviour) {
                ...
                case PSP_GE_SIGNAL_RET: {
                    ...
                    currentList->signal = behaviour;
                    if (currentList->stackptr == 0) {
                        ERROR_LOG_REPORT(G3D, "Signal with Return: stack empty. signal/end: %04x %04x", signal,
                                         enddata);
                    } else {
                        // TODO: This might save/restore other state...
                        auto &stackEntry    = currentList->stack[--currentList->stackptr];
                ...
                default:
                    ERROR_LOG_REPORT(G3D, "UNKNOWN Signal UNIMPLEMENTED %i ! signal/end: %04x %04x", behaviour, signal,
                                     enddata);
                    break;
            }
        ...
        case GE_CMD_FINISH:
            switch (currentList->signal) {
                ...
                default:
                    ...
                    if (currentList->interruptsEnabled &&
                        __GeTriggerInterrupt(currentList->id, currentList->pc, startingTicks + cyclesExecuted)) {
                        currentList->pendingInterrupt = true;
                    } else {
