...
// Note: __MAC_10_7 is defined on 10.7+.
     ...
     defined(__APPLE__)) /* GCC 4.4 provides <mutex>, except on these platforms: */                                    \
...
template <class Mutex> class unique_lock {
  public:
    ...
    unique_lock(unique_lock &&other) : pm(NULL), owns(false)
    {
#else
    unique_lock(const unique_lock &u) : pm(NULL), owns(false)
    {
        // ugly const_cast to get around lack of rvalue references
        unique_lock &other = const_cast<unique_lock &>(u);
#endif
