...
void PointerWrap::Do(std::string &x)
{
    ...
    switch (mode) {
        ...
        case MODE_MEASURE:
            break;
        case MODE_VERIFY:
            _dbg_assert_msg_(COMMON, !strcmp(x.c_str(), (char *)*ptr),
...
void PointerWrap::DoMarker(const char *prevName, u32 arbitraryNumber)
{
    ...
    if (mode == PointerWrap::MODE_READ && cookie != arbitraryNumber) {
        PanicAlert("Error: After \"%s\", found %d (0x%X) instead of save marker %d (0x%X). Aborting savestate load...",
...
CChunkFileReader::Error CChunkFileReader::LoadFile(const std::string &_rFilename, int _Revision,
                                                   const char *_VersionString, u8 *&_buffer, size_t &sz,
...
CChunkFileReader::Error CChunkFileReader::SaveFile(const std::string &_rFilename, int _Revision,
                                                   const char *_VersionString, u8 *buffer, size_t sz)
{
