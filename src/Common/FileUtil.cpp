...
#endif
    if (result < 0) {
        WARN_LOG(COMMON, "IsDirectory: stat failed on %s: %s", filename.c_str(), GetLastErrorMsg());
        return false;
    }
...
bool Delete(const std::string &filename)
{
    ...
    if (!DeleteFile(ConvertUTF8ToWString(filename).c_str())) {
        WARN_LOG(COMMON, "Delete: DeleteFile failed on %s: %s", filename.c_str(), GetLastErrorMsg());
        return false;
    }
#else
    if (unlink(filename.c_str()) == -1) {
        WARN_LOG(COMMON, "Delete: unlink failed on %s: %s", filename.c_str(), GetLastErrorMsg());
        return false;
    }
#endif
...
bool Rename(const std::string &srcFilename, const std::string &destFilename)
{
    ...
    if (rename(srcFilename.c_str(), destFilename.c_str()) == 0)
        return true;
#endif
    ERROR_LOG(COMMON, "Rename: failed %s --> %s: %s", srcFilename.c_str(), destFilename.c_str(), GetLastErrorMsg());
    return false;
}
...
bool Copy(const std::string &srcFilename, const std::string &destFilename)
{
    ...
    ERROR_LOG(COMMON, "Copy: failed %s --> %s: %s", srcFilename.c_str(), destFilename.c_str(), GetLastErrorMsg());
    return false;
#else
...
u64 GetSize(const int fd)
{
    struct stat64 buf;
    if (fstat64(fd, &buf) != 0) {
        ERROR_LOG(COMMON, "GetSize: stat failed %i: %s", fd, GetLastErrorMsg());
        return 0;
    }
    return buf.st_size;
}
...
u64 GetSize(FILE *f)
{
    ...
    if (fseeko(f, 0, SEEK_END) != 0) {
        ERROR_LOG(COMMON, "GetSize: seek failed %p: %s", f, GetLastErrorMsg());
        return 0;
    }
    u64 size = ftello(f);
...
bool CreateEmptyFile(const std::string &filename)
{
    ...
    if (!pFile) {
        ERROR_LOG(COMMON, "CreateEmptyFile: failed %s: %s", filename.c_str(), GetLastErrorMsg());
        return false;
    }
    fclose(pFile);
    return true;
...
std::string GetCurrentDir()
{
    ...
    if (!(dir = __getcwd(NULL, 0))) {
        ...
        ERROR_LOG(COMMON, "GetCurrentDirectory failed: %s", GetLastErrorMsg());
        return NULL;
    }
...
IOFile::IOFile() : m_file(NULL), m_good(true)
{
}
...
IOFile::IOFile(const std::string &filename, const char openmode[]) : m_file(NULL), m_good(true)
{
