...
void CheckGLExtensions()
{
    ...
    if (cvendor) {
        ...
        if (vendor == "NVIDIA Corporation" || vendor == "Nouveau" || vendor == "nouveau") {
            gl_extensions.gpuVendor = GPU_VENDOR_NVIDIA;
        } else if (vendor == "Advanced Micro Devices, Inc." || vendor == "ATI Technologies Inc.") {
            gl_extensions.gpuVendor = GPU_VENDOR_AMD;
        } else if (vendor == "Intel" || vendor == "Intel Inc." || vendor == "Intel Corporation" ||
    ...
    ILOG("GPU Vendor : %s ; renderer: %s version str: %s ; GLSL version str: %s", cvendor, renderer ? renderer : "N/A",
         versionStr ? versionStr : "N/A", glslVersionStr ? glslVersionStr : "N/A");
    ...
    gl_extensions.OES_texture_npot =
        strstr(extString, "GL_ARB_texture_non_power_of_two") != 0 &&
    ...
    gl_extensions.ARB_shader_image_load_store = (strstr(extString, "GL_ARB_shader_image_load_store") != 0) ||
                                                (strstr(extString, "GL_EXT_shader_image_load_store") != 0);
    ...
    gl_extensions.OES_packed_depth_stencil =
        (strstr(extString, "GL_OES_packed_depth_stencil") != 0) || gl_extensions.GLES3;
    ...
    if (strstr(extString, "GL_ARB_ES2_compatibility")) {
        ...
        for (int st = 0; st < 2; st++) {
            for (int p = 0; p < 6; p++) {
                glGetShaderPrecisionFormat(shaderTypes[st], precisions[p], gl_extensions.range[st][p],
