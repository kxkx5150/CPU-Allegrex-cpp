...
bool IsAcceptKeyCode(int keyCode)
{
    if (confirmKeys.empty()) {
        return keyCode == NKCODE_SPACE || keyCode == NKCODE_ENTER || keyCode == NKCODE_Z ||
               keyCode == NKCODE_BUTTON_A || keyCode == NKCODE_BUTTON_CROSS || keyCode == NKCODE_BUTTON_1;
    } else {
...
bool IsEscapeKeyCode(int keyCode)
{
    if (cancelKeys.empty()) {
        return keyCode == NKCODE_ESCAPE || keyCode == NKCODE_BACK || keyCode == NKCODE_BUTTON_CIRCLE ||
               keyCode == NKCODE_BUTTON_B || keyCode == NKCODE_BUTTON_2;
    } else {
...
void Choice::Draw(UIContext &dc)
{
    ...
    if (atlasImage_ != -1) {
        ...
        if (centered_) {
            ...
            if (iconImage_ != -1) {
                dc.Draw()->DrawImage(iconImage_, bounds_.x2() - 32 - paddingX, bounds_.centerY(), 0.5f, style.fgColor,
                                     ALIGN_CENTER);
            }
    ...
    if (selected_) {
        dc.Draw()->DrawImage(dc.theme->checkOn, bounds_.x2() - 40, bounds_.centerY(), 1.0f, style.fgColor,
                             ALIGN_CENTER);
    }
}
...
void InfoItem::Draw(UIContext &dc)
{
    ...
    dc.DrawText(rightText_.c_str(), bounds_.x2() - paddingX, bounds_.centerY(), 0xFFFFFFFF,
                ALIGN_VCENTER | ALIGN_RIGHT);
    // 	dc.Draw()->DrawImageStretch(dc.theme->whiteImage, bounds_.x, bounds_.y, bounds_.x2(), bounds_.y + 2,
    // dc.theme->itemDownStyle.bgColor);
}
...
void ItemHeader::Draw(UIContext &dc)
{
    ...
    dc.Draw()->DrawImageStretch(dc.theme->whiteImage, bounds_.x, bounds_.y2() - 2, bounds_.x2(), bounds_.y2(),
                                0xFFFFFFFF);
}
...
{
    dc.SetFontStyle(dc.theme->uiFont);
    dc.DrawText(text_.c_str(), bounds_.x + 12, bounds_.centerY(), dc.theme->popupTitle.fgColor,
...
void Button::Draw(UIContext &dc)
{
    ...
    if (imageID_ != -1 && text_.empty()) {
        ...
        if (imageID_ != -1) {
            const AtlasImage &img = dc.Draw()->GetAtlas()->images[imageID_];
            dc.Draw()->DrawImage(imageID_, bounds_.centerX() - tw / 2 - 5 - img.w / 2, bounds_.centerY(), 1.0f,
